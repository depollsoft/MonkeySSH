...
void registerNotificationNavigationTests() {
  group('notification_navigation', () {
    testWidgets(
      'scheduler holds locked taps and delivers latest after unlock',
      (tester) async {
        var unlocked = false;
        final opened = <String>[];
        final queue =
    ...
    for (final kind in AcpNotificationKind.values) {
      testWidgets(
        'ACP $kind opens its terminal above connections on every tap',
        (tester) async {
          final locations = <Uri>[];
          final router = GoRouter(
            initialLocation: '/settings',
            routes: [
              GoRoute(
                ...
                builder: (context, state) => Scaffold(
                  body: Text(
                    'home:${state.uri.queryParameters['tab'] ?? 'hosts'}',
                  ),
                ),
              ),
              GoRoute(
    ...
    testWidgets('terminal notification with mux window opens that window', (
      tester,
    ) async {
      final terminalLocations = <String>[];
      final router = GoRouter(
      ...
      openTerminalNotificationStack(
        router: router,
        payload: const TerminalNotificationPayload(
          hostId: 7,
          connectionId: 21,
          tmuxSessionName: 'work',
          tmuxWindowIndex: 3,
      ...
      expect(
        terminalLocations.single,
        '/terminal/7?connectionId=21&tmuxSession=work&tmuxWindow=3&tmuxWindowId=%409&notificationTap=tap-3',
      );
