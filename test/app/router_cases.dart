...
void registerRouterTests() {
  group('router', () {
    ...
    group('routerProvider', () {
      ...
      test(
        'intentionally returns a new GoRouter instance when authState changes '
        'to reset protected back-stack history',
        () async {
      ...
      test(
        'persistent router refresh behavior is deferred until notification '
        'deep-links and lock back-stack safety are proven together',
        () async {
          container.read(authStateProvider);
          await pumpEventQueue();
          expect(container.read(authStateProvider), AuthState.notConfigured);
      ...
      test(
        'new GoRouter after unlocking allows navigation past /lock',
        () async {
          // Start locked.
          when(() => authService.isAuthEnabled()).thenAnswer((_) async => true);
          when(() => authService.verifyPin(any()))
          ...
          expect(
            ...
            isNull,
            reason:
                'After unlocking, navigation to any non-lock route is allowed.',
          );
        },
      );
    });
