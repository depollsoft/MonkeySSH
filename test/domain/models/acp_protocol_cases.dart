  ...
  group('acp_protocol', () {
    ...
    test('permission and configuration lists tolerate malformed entries', () {
      for (final raw in [
        ...
        expect(
          AcpPermissionRequest.fromJson({'options': raw}).options,
          isEmpty,
        );
    ...
    test('normalizes Grok metadata-only reasoning modes', () {
      ...
      expect(
        result.modes?.availableModes.first.description,
        contains('quality'),
      );
    });
    ...
    test('bounds provider plan entry count and content', () {
      ...
      expect(
        update.entries.first.content,
        hasLength(acpMaxPlanEntryCharacters),
      );
    });
