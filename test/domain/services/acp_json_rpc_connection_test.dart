...
import 'acp_client_cases.dart';
import 'acp_ssh_exec_transport_cases.dart';
...
void main() {
  group('acp_json_rpc_connection', () {
    ...
    test(
      'close awaits cleanup already started by a protocol failure',
      () async {
        final transport = _GatedTransport();
        final connection = AcpJsonRpcConnection(transport: transport);
    ...
    test(
      'decoded input still validates size, version and request IDs',
      () async {
        for (final frame in [
    ...
    test(
      'routes mixed responses, notifications, and server requests',
      () async {
        final transport = _MemoryTransport();
        final connection = AcpJsonRpcConnection(
          transport: transport,
  ...
  registerAcpClientTests();
  registerAcpSshExecTransportTests();
}
