...
void registerAcpNotificationTargetTests() {
  group('acp_notification_target', () {
    ...
    test(
      'finds owning connection after focus moved to another window',
      () async {
        final queried = <int>[];
    ...
    test(
      'keeps focused preference when another lookup finishes first',
      () async {
        final pending = <int, Completer<List<TmuxWindow>>>{};
        final result = resolveAcpNotificationConnection(
          target: target,
    ...
    test(
      'does not select a window belonging to a different provider',
      () async {
        expect(
          await resolveAcpNotificationConnection(
            target: target,
