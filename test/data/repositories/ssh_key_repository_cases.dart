...
void registerSshKeyRepositoryTests() {
  group('ssh_key_repository', () {
    late AppDatabase db;
    ...
    group('KeyRepository', () {
        ...
        for (final damagedField in ['privateKey', 'passphrase', 'both']) {
          test('$corruption in $damagedField recovers per field', () async {
            ...
            final storedPrivateKey = privateKeyUnreadable
                ? corruptPrivateKey
                : await encryptionService.encryptRequired(
            ...
            expect(
              ...
              isNot(contains(corruptPrivateKey)),
            );
            expect(
              diagnostics.exportText(),
              isNot(contains(corruptPassphrase)),
            );
      ...
      test(
        'new secrets may literally start with the envelope prefix',
        () async {
      ...
      test('legacy key migration does not overwrite newer writes', () async {
        ...
        final newerEncryptedPrivateKey = await encryptionService
            .encryptNullable('newer-open-ssh-material...');
      ...
      test(
        'insert does not double-encrypt a pre-encrypted passphrase',
        () async {
          const privateKey = 'fixture-open-ssh-material...';
          const passphrase = 'my-passphrase';
          final id = await repository.insert(
      ...
      test('watchAll retains unreadable stored keys', () async {
        ...
        final unreadablePrivateKey = await otherEncryptionService
            .encryptRequired('unreadable-open-ssh-material...');
        await db
            .into(db.sshKeys)
            .insert(
              SshKeysCompanion.insert(
