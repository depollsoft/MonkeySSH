...
void registerHostRepositoryTests() {
  group('host_repository', () {
    late AppDatabase db;
    ...
    group('HostRepository', () {
      ...
      test('all host read paths tolerate a corrupt envelope without rewriting it', () async {
      ...
      test('getById tolerates a password encrypted with a lost key', () async {
        ...
        final encryptedPassword = await previousEncryptionService
            .encryptNullable('unrecoverable-secret');
        final id = await db
            .into(db.hosts)
            .insert(
              HostsCompanion.insert(
      ...
      test(
        'legacy password migration does not overwrite newer writes',
        () async {
          ...
          final newerEncryptedPassword = await encryptionService
              .encryptNullable('newer-secret');
      ...
      test(
        'duplicate copies all host configuration and port forwards',
        () async {
          ...
          expect(
            duplicateHost.skipJumpHostOnSsids,
            sourceHost.skipJumpHostOnSsids,
          );
          ...
          expect(
            duplicateHost.terminalThemeDarkId,
            sourceHost.terminalThemeDarkId,
          );
          expect(
            duplicateHost.terminalFontFamily,
            sourceHost.terminalFontFamily,
          );
          expect(
            duplicateHost.autoConnectCommand,
            sourceHost.autoConnectCommand,
          );
          expect(
          ...
          expect(
            _hostConfiguration(duplicateHost),
            _hostConfiguration(sourceHost),
          );
          ...
          expect(
            databaseTunnel.createdAt,
            isNot(DateTime(2020, 4, 5, 6, 7, 8)),
          );
          ...
          final sourcePortForwards =
              await (db.select(db.portForwards)..where(
                    (portForward) => portForward.hostId.equals(sourceHostId),
                  ))
                  .get();
      ...
      test(
        'favorite field updates do not double-encrypt the password',
        () async {
      ...
      test(
        'updateLastConnected does not double-encrypt the password',
        () async {
      ...
      test('setAutoForwardPorts persists automatic port detection', () async {
        ...
        expect(
          await repository.setAutoForwardPorts(id, enabled: false),
          isTrue,
        );
      ...
      test('setAutoForwardPorts returns false when host not exists', () async {
        expect(
          await repository.setAutoForwardPorts(999, enabled: true),
          isFalse,
        );
      });
