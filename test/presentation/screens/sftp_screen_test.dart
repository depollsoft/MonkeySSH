  ...
  group('SFTP path helpers', () {
    ...
    for (final failure in <Object>[
      ...
      for (final hasEarlierSuccess in [false, true]) {
        testWidgets('upload handles ${failure.runtimeType} and stops the batch'
          ...
          final failedClosed = (await tester.runAsync(
            () async => Completer<void>(),
          ))!;
          when(failedFile.close)
              .thenAnswer((_) async => failedClosed.complete());
          ...
          await tester.runAsync(() async {
            await tester.tap(find.byTooltip('Upload files'));
            await failedClosed.future.timeout(const Duration(seconds: 5));
          });
          await tester.pumpAndSettle();
          expect(
            find.descendant(
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final reading = signals.reading;
        final stream = signals.stream;
        ...
        await tester.pump();
        final cachedFile = directory
            .listSync(recursive: true)
            .whereType<File>()
            .single;
        expect(find.text('Loading video preview'), findsOneWidget);
        // Subscribe before cancellation so cache deletion is the completion
        // signal even when it happens before the dialog's exit animation.
        late Future<FileSystemEvent> deleted;
        await tester.runAsync(() async {
          deleted = cachedFile.parent
              .watch(events: FileSystemEvent.delete)
              .firstWhere((event) => event.path == cachedFile.path)
