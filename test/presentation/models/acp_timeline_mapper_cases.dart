...
void registerAcpTimelineMapperTests() {
  group('acp_timeline_mapper', () {
    test(
      'builds previews from only the newest lines in chronological order',
      () {
    ...
    test('maps ordered user content, assistant markdown, and tool calls', () {
      ...
      expect(
        (user.parts[1] as p.AcpResourcePart).resource.uri,
        'file:///a.txt',
      );
    ...
    test(
      'groups Claude parent-linked updates under the launching subagent',
      () {
        final timeline = AcpTimeline(
          entries: [
            AcpToolCallEntry(
      ...
      final timeline = AcpTimeline(
        entries: [
          AcpToolCallEntry(
            ...
            content: [
              AcpToolContentBlock(
                content: AcpTextContent(jsonEncode(rawOutput)),
              ),
            ],
          ),
        ],
    ...
    test('separates distant edits into minimal unified hunks', () {
      final oldLines = [
        for (var index = 1; index <= 30; index++) 'line $index',
      ];
...
'''
    ...
    test(
      'reports structure consistently with formatted JSON and plain text',
      () {
