...
import 'host_edit_view_model_lifetime_cases.dart';
...
void main() {
  group('host_edit_view_model', () {
    group('HostEditViewModel', () {
      test(
        ...
        () async {
          ...
          expect(
            await settings.getJson(SettingKeys.agentLaunchPresets),
            isNull,
          );
          await settings.setJson(SettingKeys.agentLaunchPresets, {
            '$id': legacy,
      ...
      test('reports stable validation targets and messages', () {
        ...
        expect(
          viewModel.validateDraft(
            _draft(startupMode: HostStartupMode.monkeyMux),
          ),
          isA<HostEditValidationIssue>()
              .having(
                (issue) => issue.target,
    ...
    group('tmux status bar helpers', () {
      test('round trip hidden status bar checkbox flags', () {
        ...
        expect(
          stripTmuxDisableStatusBarCommand(storedFlags),
          '-f ~/.tmux.conf',
        );
        expect(
  ...
  registerHostEditViewModelLifetimeTests();
}
