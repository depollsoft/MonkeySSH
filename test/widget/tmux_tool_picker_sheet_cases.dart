  ...
  group('tmux_tool_picker_sheet', () {
    group('AgentToolIcon', () {
      test('window identity color depends only on selection', () {
        ...
        expect(
          agentWindowIdentityColor(scheme, isActive: true),
          scheme.primary,
        );
        expect(
      ...
      testWidgets('renders a branded svg for a known tool name', (
        tester,
      ) async {
      ...
      testWidgets('renders a branded svg for a known tool enum', (
        tester,
      ) async {
    ...
    group('TmuxToolPickerSheet', () {
      ...
      test('omits Gemini while keeping Antigravity selectable', () async {
        final tools = await resolveTmuxNewWindowTools(
          Future.value(AgentLaunchTool.values.toSet()),
        );
        expect(tools.map((tool) => tool.label), isNot(contains('Gemini CLI')));
        expect(
          tools.singleWhere((tool) => tool == AgentLaunchTool.antigravity),
          AgentLaunchTool.antigravity,
        );
      });

      test('renders only detected tools', () async {
        final tools = await resolveTmuxNewWindowTools(
          Future.value({AgentLaunchTool.claudeCode, AgentLaunchTool.codex}),
        );
        expect(
          tools.where((t) => t == AgentLaunchTool.claudeCode),
          hasLength(1),
        );
        expect(tools.where((t) => t == AgentLaunchTool.codex), hasLength(1));
        expect(tools, isNot(contains(AgentLaunchTool.copilotCli)));
        expect(tools.map((tool) => tool.label), isNot(contains('Gemini CLI')));
        expect(tools, isNot(contains(AgentLaunchTool.openCode)));
      });
      ...
      testWidgets('shows fallback message when detection fails', (
        tester,
      ) async {
        final completer = Completer<Set<AgentLaunchTool>>();
        await tester.pumpWidget(
      ...
      testWidgets('invokes callback when a detected tool is tapped', (
        ...
        expect(chosen, AgentLaunchTool.antigravity);
      });

      test('moves the preferred tool to the top of the list', () async {
        final tools = await resolveTmuxNewWindowTools(
          Future.value({AgentLaunchTool.claudeCode, AgentLaunchTool.codex}),
          preferredTool: AgentLaunchTool.codex,
        );
        expect(
          tools.indexOf(AgentLaunchTool.codex),
          lessThan(tools.indexOf(AgentLaunchTool.claudeCode)),
        );
      });
    });
  });
