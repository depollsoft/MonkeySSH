...
void registerMonkeyTerminalViewLayoutTests() {
  group('monkey_terminal_view_layout', () {
    ...
    group('text attribute rendering', () {
      test('conceal (SGR 8) does not draw the glyph', () async {
        ...
        expect(
          visible,
          greaterThan(0),
          reason: 'visible glyph should be drawn',
        );
        expect(concealed, 0, reason: 'concealed glyph should not be drawn');
      });

      test('foreground picture cache reuses unchanged lines and re-renders changes', () async {
        final theme = TerminalThemes.defaultDarkTheme.toXtermTheme();
      ...
      test('style-run batching falls back to per-cell for combining marks, '
        ...
        expect(
          ...
          reason: 'non-batchable cells must render identically to the per-cell path',
        );
      });
    ...
    group('explicit cell background rendering', () {
      ...
      test(
        'keeps bright-black backgrounds readable across built-in themes',
        () {
      ...
      test('renders a full-width prompt background painted with real cells', () async {
