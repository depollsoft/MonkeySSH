...
void registerTmuxWindowNavigatorTests() {
  group('tmux_window_navigator', () {
    ...
    group('shared mux presentation', () {
      test('synthetic numbers ignore activity order and skip server-owned sessions', () {
    ...
    group('shared recent sessions', () {
      ...
      Widget recentHost({
        ...
        AgentWindowModePreference mode =
          ...
          testWidgets(
            ...
            (tester) async {
              ...
              when(
                ...
                (_) =>
                    Stream.value(DiscoveredSessionsResult(sessions: const [])),
              );
              when(
              ...
              final expectedNative =
                  (mode == AgentWindowModePreference.preferNative) !=
                  forcePicker;
              if (forcePicker) {
    ...
    group('tmux navigator UI', () {
      ...
      testWidgets(
        'loads AI session providers only after expanding the section',
        (tester) async {
          const tmuxSessionName = 'main';
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          when(() => tmuxService.detectInstalledAgentTools(session)).thenAnswer(
            (_) async => const <AgentLaunchTool>{AgentLaunchTool.pi},
          );
      ...
      testWidgets(
        'stops showing an indefinite spinner after repeated empties',
        (tester) async {
          const tmuxSessionName = 'main';
      ...
      test('maps every verified ACP adapter to its terminal tool', () {
        ...
        expect(
          mapping[AgentLaunchTool.openCode],
      ...
      test('app preference opens native chat without prompting', () {
        final mode = preferredAgentWindowMode(
          preference: AgentWindowModePreference.preferNative,
        );
        expect(
          mode,
          isNotNull,
          reason: 'A resolved preference does not open the mode chooser',
        );
        expect(mode, AgentWindowMode.nativeAcp);
        expect(
          ...
          isA<TmuxNewAcpSessionAction>(),
        );
      });

      test('free app preference opens terminal without prompting', () {
        final mode = preferredAgentWindowMode(
          preference: AgentWindowModePreference.preferTerminal,
        );
        expect(
          mode,
          isNotNull,
        ...
        expect(mode, AgentWindowMode.terminal);
        expect(
          agentWindowActionForMode(
            mode: mode!,
      ...
      test('free tool without ACP support opens a terminal directly', () {
        final mode = preferredAgentWindowMode(
          preference: AgentWindowModePreference.askEveryTime,
        ...
        expect(mode, AgentWindowMode.terminal);
        final action = agentWindowActionForMode(
          mode: mode!,
          tool: AgentLaunchTool.claudeCode,
          startClisInYoloMode: false,
          providerId: null,
        );
        expect(action, isA<TmuxNewWindowAction>());
        expect(
          (action as TmuxNewWindowAction).agentTool,
      ...
      testWidgets('MonkeyMux navigator lists and opens native ACP sessions', (
        ...
        final nativeRow = find.byKey(
          ValueKey('native-acp-session-${key.value}'),
        );
        expect(
          find.text('Copilot CLI · …/monkeyssh · working'),
          findsOneWidget,
        );
        expect(
