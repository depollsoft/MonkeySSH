  ...
  group('terminal_screen_selection', () {
    ...
    group('tmux window snapshot matching', () {
      ...
      test('missing IDs match by index and present IDs do not fall back', () {
        ...
        expect(
          shouldRefreshTmuxThemeAfterWindowChange([noId], [noId]),
          isFalse,
        );
        expect(
          shouldRefreshTmuxThemeAfterWindowChange([noId], [first]),
          isTrue,
        );
        expect(
          shouldRefreshTmuxThemeAfterWindowChange([first], [noId]),
          isTrue,
        );
      });
    ...
    group('trimTerminalFilePathCandidate', () {
      ...
      test(
        'drops wrapped result-count suffixes after unmatched parentheses',
        () {
          expect(
            trimTerminalFilePathCandidate(
      ...
      test(
        'drops wrapped result-count suffixes from dotless explicit paths',
        () {
    ...
    group('resolveTerminalFilePathVerificationCandidates', () {
      ...
      test(
        'keeps balanced trailing brackets that are part of the filename',
        () {
          expect(
            resolveTerminalFilePathVerificationCandidates(
    ...
    group('hasAmbiguousTerminalFilePathParsing', () {
      test('returns false for ordinary explicit paths with a final known extension', () {
        expect(
      ...
      test(
        ...
        () {
          expect(
            hasAmbiguousTerminalFilePathParsing(
              '/srv/app/archive.tar.gzbackup',
            ),
            isTrue,
          );
    ...
    group('resolveTerminalUploadPickerRequest', () {
      test(
        'allows selecting multiple images or videos for terminal uploads',
        () {
    ...
    group('deliverTerminalAttachmentPasteSegments', () {
      ...
      test(
        'stops after input arrives during the first control await',
        () async {
          var inputGeneration = 0;
          var waits = 0;
          final injected = <String>[];
    ...
    group('refreshTerminalBracketedPasteModeFromMuxWindows', () {
      ...
      test(
        'can conservatively paste without mutating cached terminal mode',
        () {
          final output = <String>[];
          final terminal = Terminal(onOutput: output.add)
    ...
    group('terminalAttachmentPasteTargetsCurrentMuxWindow', () {
      test(
        'rejects an optimistic window switch before its snapshot arrives',
        () {
          expect(
    ...
    group('detectTerminalFilePathAtTextOffset', () {
      ...
      test(
        'detects tilde paths split across unindented continuation lines',
        () {
          const text =
      ...
      test(
        'detects paths split across lines ending with a scrollbar glyph',
        () {
          const text =
      ...
      test(
        'detects paths split across mixed scrollbar thumb and track rows',
        () {
          const text =
    ...
    group('resolveTerminalFilePathSegmentOnRow', () {
        ...
        () {
          ...
          expect(
            resolveTerminalFilePathSegmentOnRowForPath(
              ...
              rowText: rowText,
              rowStartOffset: snapshotText.indexOf(rowText),
              rowColumnOffsets: List<int>.generate(
                rowText.length + 1,
                (i) => i,
              ),
      ...
      test(
        'resolves guided single-character third-line path continuations',
        () {
          const snapshotText =
              '│ Read ~/Code/flutty.worktrees/fix-swipe-keyboard-typing/lib/\n'
              '│ presentation/widgets/terminal_text_input_handler.dar\n'
    ...
    group('isLaunchableTerminalUri', () {
      test('allows supported external link schemes', () {
        expect(
          isLaunchableTerminalUri(Uri.parse('https://example.com')),
          isTrue,
        );
        expect(
    ...
    group('terminalRowMayContainPath', () {
      ...
      test('returns true for a row with an absolute path prefix', () {
        expect(
          terminalRowMayContainPath(
            buildLineWithText('ls /var/log/app.log'),
            80,
          ),
          isTrue,
        );
      ...
      test('returns true for a row containing only a slash', () {
        expect(
          terminalRowMayContainPath(buildLineWithText('cd /'), 80),
          isTrue,
        );
      });

      test(
        'returns false for a row with plain text and no path characters',
        () {
          expect(
            terminalRowMayContainPath(
              buildLineWithText('echo hello world'),
              80,
            ),
            isFalse,
          );
        ...
        expect(
          resolveTerminalPathTouchTargetTap(const Offset(18, 30), const [
            (
              ...
              touchRect: Rect.fromLTRB(14, 10, 114, 46),
            ),
          ]),
          '/var/log/app.log',
        );
    ...
    group('shouldResolveTerminalTapLinks', () {
      ...
      test(
        'blocks link taps while the native selection overlay is visible',
        () {
          expect(
    ...
    group('terminalSensitivePromptTextBeforeCursor', () {
      test(
        'rejects the entire overlong wrapped prefix, not just its suffix',
        () {
