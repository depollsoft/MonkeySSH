...
void registerTerminalScreenLayoutTests() {
  group('terminal_screen_layout', () {
    group('terminal layout helpers', () {
      ...
      test('displays the confirmed selection title and agent tool', () {
        const windows = <TmuxWindow>[
      ...
      test(
        'omits duplicate or blank tmux window titles in the handle label',
        () {
      ...
      test(
        'keeps verifying the existing tmux session without a preference',
        () {
          expect(
      ...
      test('defaults missing remote mux backend to automatic startup', () {
        ...
        expect(
          resolveRemoteMuxStartupBackend(
      ...
      test(
        'resolves preferred tmux session name before remote verification',
        () {
          expect(
      ...
      test(
        'shrinks shell completion popup rather than covering cursor line',
        () {
          final layout = resolveShellCompletionPopupLayout(
