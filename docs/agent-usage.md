...
Claude Code, Codex, Antigravity, Grok Build, and native Pi can show remaining account
...
tools need a known active provider. Native Pi reads the provider from its live
model selection and filters Pi's saved-account quotas to that provider. Rings
follow provider changes and disappear when the selection is unknown. Anthropic
