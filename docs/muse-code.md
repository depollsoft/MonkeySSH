...
Install Muse on a macOS or Linux SSH host through Agent Management or
[Meta's installer](https://dev.meta.ai/install.sh). Run `muse login` in a terminal
to sign in. Native Windows hosts are marked unavailable; a WSL host reached
through SSH follows the Linux path.
...
Recent sessions read the local index and bounded log heads under
...
separately. `muse serve` speaks MSP, so it cannot be launched directly as ACP.
