...
export 'acp_thread_projection.dart' show acpUserPromptSummary;
...
class _AcpMessageThreadState extends State<AcpMessageThread> {
  ...
  List<AcpThreadChild> _threadChildren = const [];
  ...
  void _syncThreadChildren() {
    ...
    if (entriesChanged) {
      ...
      if (widget.followTail) {
        final tail = projectAcpThreadWindow(widget.entries);
        ...
        _threadChildren = buildAcpThreadChildren(
          widget.entries,
          startEntryIndex: 0,
        );
      } else {
  ...
  void _revealEarlierTranscriptPage({
    ...
    if (prefix.isNotEmpty) {
      _threadChildren = <AcpThreadChild>[...prefix, ..._threadChildren];
      _rebuildThreadChildIndexes();
    }
  ...
  Future<void> _scrollEntryIntoView(int entryIndex) async {
    ...
    if (absoluteTargetChildIndex == null &&
      ...
      setState(() {
        final prefix = buildAcpThreadChildren(
          widget.entries,
