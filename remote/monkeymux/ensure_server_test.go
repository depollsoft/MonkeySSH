...
func writeStaleSessionLock(t *testing.T) (session, path string) {
	t.Helper()
...
func TestTakeoverGuardIsExclusive(t *testing.T) {
	isolateTestRuntime(t)
	_, path := writeStaleSessionLock(t)

	release, acquired, err := acquireTakeoverGuard(path)
	if err != nil || !acquired {
		t.Fatalf("first acquire: acquired=%v err=%v", acquired, err)
	}
	if _, again, err := acquireTakeoverGuard(path); err != nil || again {
		t.Fatalf("a held guard was acquired again: acquired=%v err=%v", again, err)
	}
	release()
	release2, acquired, err := acquireTakeoverGuard(path)
	if err != nil || !acquired {
		t.Fatalf("acquire after release: acquired=%v err=%v", acquired, err)
	}
	release2()
	if _, err := os.Stat(takeoverGuardPath(path)); err != nil {
		t.Fatalf("the guard file must stay in place: %v", err)
	}
}
...
func TestReclaimSessionFileWaitsForTakeoverGuard(t *testing.T) {
	...
	// Another helper is mid-reclaim.
	release, acquired, err := acquireTakeoverGuard(path)
	if err != nil || !acquired {
		t.Fatalf("acquire guard: acquired=%v err=%v", acquired, err)
	}
	...
	if _, again, err := acquireTakeoverGuard(path); err != nil || !again {
		t.Fatalf("the guard was not released after the reclaim: acquired=%v err=%v", again, err)
	}
}

func TestAcquireSessionLockIgnoresUnlockedGuardFile(t *testing.T) {
	isolateTestRuntime(t)
	session, path := writeStaleSessionLock(t)
	// The guard file of a helper that died mid-reclaim: the operating system
	// dropped its lock, so the file itself means nothing.
	guard := takeoverGuardPath(path)
	if err := os.WriteFile(guard, []byte("residue"), 0o600); err != nil {
	...
	if err != nil || record.pid != os.Getpid() {
		t.Fatalf("lock after reclaim: %+v, %v", record, err)
	}
}

func TestGCKeepsTakeoverGuard(t *testing.T) {
	isolateTestRuntime(t)
	_, path := writeStaleSessionLock(t)
	guard := takeoverGuardPath(path)
	if err := os.WriteFile(guard, []byte("residue"), 0o600); err != nil {
	...
	if _, err := os.Stat(guard); err != nil {
		t.Fatalf("gc removed the takeover guard: %v", err)
	}
	if _, err := os.Stat(path); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("gc kept the stale lock: %v", err)
	}
}
